...
async def resposta_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if prova["tipus"] == "final_joc":
        await update.message.reply_text(
            ...
            "🐔 Lo Corral associació cultural, Ginestar, 28 de setembre de 2025.")
# ----------------------------
# Comanda /r30 resumida (1 línia per equip amb hora inscripció)
# ----------------------------
...
async def mostrar_equips_resum(update: Update, context: ContextTypes.DEFAULT_TYPE):
    equips = carregar_equips()
    proves = carregar_proves()
    records_sheet = sheet.get_all_records()
    ...
    bloc3_ids = range(21, 31)  # Proves del bloc 3
    temps_bloc3 = [
        row["hora"] for row in records_sheet
        if row["equip"] == e and int(row["prova_id"]) in bloc3_ids
    ]
    if all(str(pid) in respostes for pid in bloc3_ids if str(pid) in proves):
        complet_bloc3 = max(temps_bloc3) if temps_bloc3 else "Desconeguda"
    else:
        complet_bloc3 = "No completat"



        # Una sola línia per equip
        msg += f"{e} | @{info['portaveu']} | {', '.join(info['jugadors'])} | Inscripció: {hora_inscripcio} | Bloc3: {complet_bloc3}\n"

    await update.message.reply_text(msg)

# Registrar la comanda al bot
